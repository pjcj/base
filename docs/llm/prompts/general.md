...
- Describe the business value or technical necessity
...
**CRITICAL:** Do not include information that is obvious from examining the
commit or running standard tools on it.

**Never include:**

- Coverage percentages or metrics (e.g., "Coverage increased from 90% to 91%")
- Test assertion counts or line counts (e.g., "Added 50 assertions")
- Implementation details visible in the diff (e.g., "Changed if to unless")
- "Before" and "after" metrics (e.g., "Was 80%, now 95%")
- Statistics that tools can generate (e.g., "10 files changed, 200 insertions")
- Specific line numbers (e.g., "Fixed line 42", "Add tests for lines 191-197")

**Why:** These details clutter the git history with noise that becomes
outdated as code evolves:

- Line numbers change as code is added or removed
- Coverage metrics can be regenerated by running tests
- Assertion counts are visible in test output
- File statistics are shown by git itself

Anyone can run coverage tools, count assertions, or view diffs. Focus on the
meaningful change (what and why), not the measurements (how much) or locations
(which lines).
...
Add tests for API endpoint routing in Request.pm. Tests verify that
/iapi/v{N}/settings paths are correctly routed to the API class.
```

#### Commit message guidelines summary
