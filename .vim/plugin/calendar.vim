...
function! Calendar(...)
  ...
  if vwinnum >= 0
    ...
    if (!exists('s:bufautocommandsset'))
      auto BufEnter *Calendar let b:sav_titlestring = &titlestring | let &titlestring = '%{strftime("%c")}' | let b:sav_wrapscan = &wrapscan | resize 9
      auto BufLeave *Calendar let &titlestring = b:sav_titlestring | let &wrapscan = b:sav_wrapscan | resize 9
  ...
  execute 'nnoremap <silent> <buffer> h  :call <SID>CalendarDoAction("<' . pnav . '")<cr>'
  execute 'nnoremap <silent> <buffer> l  :call <SID>CalendarDoAction("' . nnav . '>")<cr>'
